...
\begin{example}{Subset vs. Subspace}{subset-subspace}
    ...
    considered as a \emph{subset} of $ \mathbb{R} $, then $ \partial A = \{0, 1,
...
Let $\epsilon>0$ be given. Then, by \cref{definition:conv-equiv}, there exists $N>0$ such that $d_\infty(\vec{x}_n, \vec{x})<\epsilon$ for all $n>N$. That is, $\max\limits_{1\leq i \leq k}\vert x_i^{(n)}-x_i \vert < \epsilon$ for all $n>N$, and so, $\vert x_i^{(n)}-x_i \vert < \epsilon$ for all $n>N$ and $1\leq i \leq k$.\\
...
\begin{theorem}{Equivalent metrics have the same convergent sequences}{equiv-metric-same-conv}
    Suppose that $(X,d)$ and $\e(Y,\Tilde{d})$ are \emph{equivalent}(\cref{definition: topological-equivalence}), i.e. there exists $\epsilon>0$ such that 
    \begin{equation}
...
\begin{theorem}{The limit of a convergent sequence is unique}{limit-unique}
    ...
    \begin{proof}
        ...
        Set $\delta=\epsilon/2$.\\
        As $x_n \to x$  as $n \to \infty$ then, by \cref{definition: conv-equiv}, there exists $N = N(\delta) > 0$ such that $d(x_n, x) < \delta = \epsilon/2$ for all $n>N$. Also, there exists $\hat{N} = N(\delta) > 0$ such that $d(x_n, y) < \delta = \epsilon/2$ for all $n>\hat{N}$.\\
...
\begin{theorem}{Convergence in sub-sequences}{subsequences-conv}
    ...
    \begin{proof}
        By \cref{conv-defn}It is sufficient to show that for any $\epsilon>0$, there exists $N>0$ such that $d(x_n,x)<\epsilon$ for all $n>N$. 
...
\begin{theorem}{Completing a metric space}{complete-space}
    ...
    If $X$ is not complete:
        ...
        \end{itemize}
    \begin{proof}
        The proof can be found in Section (1.5) of Srivali and Vasudeva 
    \end{proof}
\end{theorem}
\pagebreak
\end{document}
